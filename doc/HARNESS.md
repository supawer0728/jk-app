...
이 프로젝트에서 **AI 에이전트(Claude Code · Gemini 등)의 질의·답변은 한국어로 한다.**

- 사용자와 주고받는 모든 대화(질문·설명·요약·계획·에러 안내)는 한국어를 기본으로 한다.
- 커밋 메시지·PR 본문·문서(docs) 등 산출물도 한국어를 기본으로 한다(기존 관례 유지).
- 단, 코드 식별자·API 이름·로그 태그·명령어 등 **기술 용어와 원문 인용은 원어 그대로** 둔다.
