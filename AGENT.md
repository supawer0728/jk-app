...
**AI 에이전트(Claude Code · Gemini 등)의 질의·답변은 한국어로 한다.** 대화(질문·설명·요약·계획·에러 안내)와
산출물(커밋 메시지·PR 본문·문서) 모두 한국어를 기본으로 하되, 코드 식별자·API 이름·로그 태그·명령어 등
기술 용어와 원문 인용은 원어 그대로 둔다. 자세한 내용은 [`doc/HARNESS.md`](doc/HARNESS.md)의 "소통 언어" 참고.
