...
class FakeAssetSheetRepository : AssetSheetRepository {

    // readAssetRows()가 돌려줄 셀 행 목록(0번째 = 헤더).
    var rows: List<List<String>> = emptyList()

    // 설정되어 있으면 readAssetRows()가 이 예외를 던진다(인증 필요/네트워크 오류 재현용).
    ...
    override suspend fun readAssetRows(): List<List<String>> {
        readCount++
        error?.let { throw it }
