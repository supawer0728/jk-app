...
class BenchmarkSheetRepositoryImpl(context: Context) : BenchmarkSheetRepository {
    ...
    override suspend fun readBenchmarkRows(): List<List<String>> = withContext(Dispatchers.IO) {
        if (credential.selectedAccountName == null) {
            throw BenchmarkSheetAuthException(credential.newChooseAccountIntent())
        }
        try {
            ...
            throw BenchmarkSheetAuthException(e.intent)
        }
    }
    ...
    // 시트 이름에 공백/하이픈이 있어 A1 표기에서 작은따옴표로 감싼다. 예: 'JK-APP raw'!X2:AC101
    ...
    companion object {
        ...
        private const val FIRST_COLUMN = "X"
        private const val LAST_COLUMN = "AC"
