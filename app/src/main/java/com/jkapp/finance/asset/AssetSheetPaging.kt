...
internal const val ASSET_SHEET_FIRST_DATA_ROW = 2
internal const val ASSET_SHEET_PAGE_SIZE = 100
...
//
// 전제: 데이터는 첫 행부터 빈 줄 없이 연속으로 채워져 있다(자산 목록). 중간에 pageSize를
// 초과하는 빈 행 구간이 있으면 그 뒤 데이터는 읽지 못하므로, 그런 시트에는 쓰지 않는다.
internal suspend fun collectPagedRows(
    pageSize: Int = ASSET_SHEET_PAGE_SIZE,
    firstRow: Int = ASSET_SHEET_FIRST_DATA_ROW,
